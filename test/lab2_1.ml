...
let test_naive_eval_fn_simple _ = 
  ...
  let v = Naive.eval (Naive.App(
    Naive.Fn(["foo"],son), [Naive.Cst(3)]
    )) [] in 
...
let test_naive_eval_fn_two _ = 
  let v = Naive.eval (
    Naive.Let("x",Naive.App(Naive.Fn(["x"],Naive.Var("x")),
      [Naive.Add(
        Naive.App(
          Naive.Fn(["x"],Naive.Mul(Naive.Cst(2),Naive.Var("x")))
          ,[Naive.Cst(3)]),
        Naive.Cst(4)
      )]
        ), 
...
let test_nameless_eval_fn_simple _ = 
  let v = Nameless.eval (Nameless.App(
    Nameless.Fn(Nameless.Add(Nameless.Cst(2),Nameless.Var(0))), [Nameless.Cst(3)]
    )) [] in 
