...
module Naive  = struct
  ...
  | Vint(vv) -> vv
  | _ -> raise (ExprError "Value Not Int")
  
  let vadd v1 v2 = match v1,v2 with
    | Vint(a),Vint(b) -> Vint(a + b)
    | _ -> raise (ExprError "vadd failed")
  let vmul v1 v2 = match v1,v2 with
  | Vint(a),Vint(b) -> Vint(a * b)
  | _ -> raise (ExprError "vadd failed")
  ...
  let rec eval expr env = 
    ...
    | App(fn,args) ->
        let (env_closure ,params, body) = match (eval fn env) with
        ...
        let vs = match (Base.List.zip params vs) with
        | Ok(vs) -> vs
...
module Nameless = struct
  ...
  let rec eval expr env = 
    ...
    | App(fn,args) -> 
      let (env_closure,expr) = match (eval fn env) with
  ...
  let rec from_naive (from:Naive.expr) cenv =
    ...
    | Naive.Fn(parameters,body) -> Fn((from_naive body (List.concat [parameters;cenv])))
    | Naive.App(fn,arguments) -> App((from_naive fn cenv),(List.map (fun e -> (from_naive e cenv)) arguments))
  ;;
  end
